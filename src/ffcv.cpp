...
int main(int argc, char* argv[])
{
    ...
    do {
        ...
        sws_scale(swsctx, decframe->data, decframe->linesize, 0, decframe->height, frame->data, frame->linesize);
        {
			...
			ostringstream os;
			os << infile << "." << nb_frames << ".jpg";
			cv::imwrite(os.str(), image);

			ostringstream os1;
			os1 << infile << "." << nb_frames << ".bin";
			auto binfile = std::fstream(os1.str(),
					std::ios::out | std::ios::binary);
			std::cout << "Linesize: " << frame->linesize[0] << ", Bin: " << os1.str()<< std::endl;
			size_t size = image.cols * image.rows * image.channels() * image.elemSize1();
			binfile.write((char *) image.data, size);
			binfile.close();
        }
